...
function renderRows(items){
  ...
  items.forEach(it =>{
    ...
    tr.innerHTML = `
      ...
      <td class="p-3"><button class="openBtn bg-indigo-600 text-white px-2 py-1 rounded" data-id="${it.id}">View</button></td>
    `;
    rows.appendChild(tr);
  });
}
...
async function fetchCompanies(){
  const params = new URLSearchParams({page:1, per_page:50, q: $('#companySearch')? $('#companySearch').value : ''});
...
function renderCompanies(items){
  ...
  items.forEach(c=>{
    ...
    card.innerHTML = `<h3 class="font-semibold">${escapeHtml(c.name)}</h3><p class="text-sm text-gray-500">${escapeHtml(c.industry||'')}</p><p class="text-sm mt-2">${escapeHtml((c.description||'').slice(0,200))}</p><div class="mt-2"><button class="companyOpen bg-indigo-600 text-white px-3 py-1 rounded" data-id="${c.id}">View</button></div>`;
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(window.PAGE_TYPE === 'companies'){
    $('#companySearch').addEventListener('input', ()=>{ fetchCompanies().then(d=>{ renderCompanies(d.items || []); $('#companyLoading').style.display='none'; }); });
    $('#exportCompanies').addEventListener('click', ()=>{ window.location = '/export/internships.csv'; });
    fetchCompanies().then(d=>{ renderCompanies(d.items || []); $('#companyLoading').style.display='none'; });
  }
});
